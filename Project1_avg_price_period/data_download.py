...
def statistic_indicators(data):
    '''Задача №7. Расширенный анализ данных
    Cтатья https://medium.com/@palajnc/calculating-the-volatility-and-return-of-stocks-with-python-cb6d90314e5a
    '''
    data_pct_list = list(data['Close'].pct_change().fillna(0)) # nan заменяем на 0
    '''
    Стандартное отклонение (Standard Deviation, SD) - это индикатор, который показывает насколько 
    сильно цены отклоняются в ту или иную сторону от средней цены на восходящем или нисходящем рынках. 
    Используется функция std() для вычисления'''
    '''
    Доходность - Данная величина представляет собой процентное изменение стоимости акции за один торговый день. 
    Оно не учитывает дивиденды и комиссии. Рассчтывается используя функцию pct_change() из пакета Pandas.
    В данным случаю мы рассчитываем среднюю доходность по цене закрытия за указанный период.
    '''
    dict_statistic_indicators = {'Cтандартное отклонение цены закрытия': float(data['Close'].std()),
                                 'Средняя доходность %': sum(data_pct_list) / len(data_pct_list)}
    return dict_statistic_indicators
